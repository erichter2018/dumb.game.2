...
class SettingsManager {
    ...
    /**
     * Add a custom trigger to a level
     * @param {string} levelName - The name of the level
     * @param {object} trigger - The trigger configuration
     */
    addCustomTrigger(levelName, trigger) {
        const normalizedName = this.normalizeLevelName(levelName);
        if (!this.settings[normalizedName]) {
            this.settings[normalizedName] = {};
        }
        if (!this.settings[normalizedName].customTriggers) {
            this.settings[normalizedName].customTriggers = [];
        }
        this.settings[normalizedName].customTriggers.push(trigger);
        this.saveSettings();
    }

    /**
     * Update a custom trigger for a level
     * @param {string} levelName - The name of the level
     * @param {number} index - The index of the trigger to update
     * @param {object} trigger - The updated trigger configuration
     */
    updateCustomTrigger(levelName, index, trigger) {
        const normalizedName = this.normalizeLevelName(levelName);
        if (this.settings[normalizedName] && this.settings[normalizedName].customTriggers) {
            this.settings[normalizedName].customTriggers[index] = trigger;
            this.saveSettings();
        }
    }

    /**
     * Remove a custom trigger from a level
     * @param {string} levelName - The name of the level
     * @param {number} index - The index of the trigger to remove
     */
    removeCustomTrigger(levelName, index) {
        const normalizedName = this.normalizeLevelName(levelName);
        if (this.settings[normalizedName] && this.settings[normalizedName].customTriggers) {
            this.settings[normalizedName].customTriggers.splice(index, 1);
            this.saveSettings();
        }
    }
