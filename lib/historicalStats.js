...
function recordLevelCompletion(levelName, durationMs, direction = 'up') {
    ...
    // Store previous best time before updating (for delta calculation)
    let previousBestTime = null;
    if (direction === 'up') {
        previousBestTime = stats.levels[levelName].allTimeBestTimeUp;
    } else if (direction === 'down') {
        previousBestTime = stats.levels[levelName].allTimeBestTimeDown;
    }
